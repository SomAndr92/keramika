...
$db = dbconn();
$category = $_GET['категория'];

$getAllCategories = $db->query("SELECT * FROM categories");
...
if (!empty($name) and !empty($price) and !empty($category) and !empty($opisaniye)) {
    ...
    if (move_uploaded_file($_FILES['photo']['tmp_name'], $uploadfile)) {
        ...
        $query = $db->query("INSERT INTO `product`(`id_categories`,`Наименование`, `Цена`, `Описание`, `Страна производства`,`Фото главное`,`Ширина`,`Длина`,`Толщина`,`Вес`) 
...
<option  value="<?=$category['ID']?>"><?=$category['категория']?></option>
